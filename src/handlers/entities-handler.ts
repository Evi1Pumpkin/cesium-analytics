...
export class EntitiesHandler {
    ...
    public getEntities(): Array<any> {
        const dataSourcesEntities: Array<any> = (this.cesiumViewer
            .dataSources as any)._dataSources.filter(
            (x: any) => x._entityCollection._entities.length > 0
        );

        return [
            ...dataSourcesEntities,
            ...(this.cesiumViewer.entities as any)._entities._array
        ];
    }
    ...
    private getPrimitivesRecursively(
        ...
        if (Array.isArray(primitiveCollection)) {
            primitiveCollection.forEach(node => {
                if (!node.hasOwnProperty('_primitives')) {
                    ...
                    if (isPrimitive) {
                        const resultNode: any = {
                            primitiveName: node.constructor.name
                        };
                ...
                if (
                    node.hasOwnProperty('_primitives') &&
                    node._primitives.length > 0
                ) {
